...
- **Astro 6**, statischer Build nach `dist/`. **Kein** Tailwind, **kein** UI-Framework,
  **keine** Runtime-Dependencies (Bewegung in Vanilla-Canvas/CSS).
- Eine einzige Seite (`src/pages/index.astro`) auf Basis eines `Layout.astro`
  ...
  (`<style is:global>`) im Layout; Client-Logik als gebündeltes `<script>` in der Seite.
- Dunkles Design mit Gold-Akzent über CSS-Custom-Properties (`--bg`, `--fg`, `--muted`,
  `--accent: #c9a96a` plus `--accent-bright/-deep`, `--cool`). Web-Fonts **Space Grotesk**
  & **Space Mono** (Google Fonts).
- Bewusst **namensfrei** und ohne Kontaktdaten: kein sichtbarer Name, keine
  E-Mail-Adresse — auch nicht in `<title>`/Meta.
- Bewegtes „Im Aufbau"-Konzept: maus-reaktiver Konstellations-Canvas, driftende Aurora +
  Blueprint-Grid, Cursor-Spotlight, rotierende SVG-Marke (`spin`/`mark-pulse`), rotierende
  Taglines und indeterminierter Fortschrittsbalken — alles per `prefers-reduced-motion`
  abschaltbar.
